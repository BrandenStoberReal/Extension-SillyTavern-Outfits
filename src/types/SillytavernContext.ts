...
export interface ISillyTavernContext {
    ...
    addOneMessage(mes: SillyTavern.ChatMessage, options?: Partial<SillyTavern.AddMessageOptions & {
        type?: string,
        insertAfter?: number,
        insertBefore?: number
    }>): void;
    ...
    executeSlashCommandsWithOptions(text: string, options?: SillyTavern.ExecuteSlashCommandsOptions): Promise<SillyTavern.SlashCommandClosureResult>;

    registerSlashCommand(command: string, callback: (...args: any[]) => any, aliases: string[], helpString?: string): void;

    executeSlashCommands(text: string, handleParserErrors?: boolean, scope?: SillyTavern.SlashCommandScope, handleExecutionErrors?: boolean, parserFlags?: Record<string, boolean>, abortController?: SillyTavern.SlashCommandAbortController, onProgress?: (done: number, total: number) => void): Promise<SillyTavern.SlashCommandClosureResult>;
    ...
    updateMessageBlock(messageId: number, message: SillyTavern.ChatMessage, options?: {
        rerenderMessage?: boolean
    }): void;

    appendMediaToMessage(mes: SillyTavern.ChatMessage, messageElement: any, adjustScroll?: boolean): void;
    ...
    parseReasoningFromString(str: string, options?: { strict?: boolean }): {
        reasoning: string,
        content: string
    } | null;
